...
class User extends Authenticatable
{
    ...
    {
        $hasLicenseRecord = $this->activeLicense()->exists();
        $hasLegacyLicense = !empty($this->license_key) && (string) $this->license_status === 'active';
        $otpRequired = !empty($this->otp_code) || !is_null($this->otp_expires_at);

        return $this->hasPaid()
            && ($hasLicenseRecord || $hasLegacyLicense)
            && (!$otpRequired || !is_null($this->otp_verified_at));
    }
}
